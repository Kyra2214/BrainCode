...
O antigo `AgentRegistry` foi removido na Fase 1 por estar órfão em produção. Especialistas continuam sendo descobertos e selecionados pelo `CapabilityRegistry` universal, `SpecialistCapabilities` e `CapabilityDiscovery`; o nome de projetos externos não é autoridade de roteamento. O roteamento real de `ChatResponseExecutor`/`WebResearchAgent` passa por capabilities e contratos, não por um registry paralelo.
