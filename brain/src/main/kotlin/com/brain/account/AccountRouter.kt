...
data class AccountRouteRequest(
    ...
    /** Contas previamente filtradas pelo serviço de disponibilidade; não é autorização. */
    val eligibleAccountIds: Set<String>? = null
) {
    init {
...
class AccountRouter {
    fun route(request: AccountRouteRequest, now: Instant): AccountRouteDecision {
        ...
        ).filter { request.eligibleAccountIds == null || it.accountId in request.eligibleAccountIds }
        ...
        val selected = candidates.firstOrNull()
            ?: return AccountRouteDecision.Unavailable(
                ...
                reasonCodes = buildList {
                    ...
                    if (request.eligibleAccountIds != null) add("MODEL_STATS_UNAVAILABLE_OR_LIMITED")
